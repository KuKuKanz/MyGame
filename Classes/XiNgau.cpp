...
static float TIME_SHOW_RESULT = 0;
...
XiNgau* XiNgau::createXiNgauAtPos(float _timeShowResult,Vec2 _pos,int _indexResult,ccResultCallBack _callBackResult){
    auto _xingau = new (std::nothrow) XiNgau();
    
    if (_xingau){
        if (!_xingau->init(_timeShowResult,_pos,_indexResult,_callBackResult)){
...
static int TIME = -1;
bool XiNgau::init(float _timeShowResult,Vec2 _pos,int _indexResult,ccResultCallBack _callBackResult){
    ...
    maxTime = MAX(_timeShowResult - (float)TIME/2,0.5);
    TIME++;
    if (TIME == 2){
        TIME = -1;
    }
...
XiNgauController* XiNgauController::create3XiNgau(float _timeShowResult,int _result1, int _result2, int _result3,ccResultCallBack _resultCallBack){
    auto _xingau = new (std::nothrow) XiNgauController();
    
    if (_xingau){
        if (!_xingau->init(_timeShowResult,_result1,_result2,_result3,_resultCallBack)){
            CC_SAFE_DELETE(_xingau);
        }else{
            _xingau->autorelease();
        }
...
bool XiNgauController::init(float _timeShowResult,int _result1, int _result2, int _result3,ccResultCallBack _resultCallBack){
    ...
    auto _spr1 = XiNgau::createXiNgauAtPos(_timeShowResult,Vec2(50,50),_result1,NULL);
    auto _spr2 = XiNgau::createXiNgauAtPos(_timeShowResult,Vec2(-50,50),_result2,NULL);
    auto _spr3 = XiNgau::createXiNgauAtPos(_timeShowResult,Vec2(0,-50),_result3,NULL);
